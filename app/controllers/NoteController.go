...
//		_, notes = noteService.ListNotes(c.GetUserId(), "", false, c.GetPage(), pageSize, defaultSortField, false, false);
		// 变成最新
		_, notes, _ = noteService.ListNotes(c.GetUserId(), "", false, c.GetPage(), 50, defaultSortField, false, false);
		if len(notes) > 0 {
			noteContent, _ = noteService.GetNoteContent(notes[0].NoteId.Hex(), userId)
		}
	}
    ...
    //revel.INFO.Print(userInfo)
    //revel.INFO.Print(c.RenderArgs["locale"])
...
func (c Note) ListNotes(notebookId string) revel.Result {
	_, notes, _ := noteService.ListNotes(c.GetUserId(), notebookId, false, c.GetPage(), pageSize, defaultSortField, false, false);
...
func (c Note) ListTrashNotes() revel.Result {
	_, notes, _ := noteService.ListNotes(c.GetUserId(), "", true, c.GetPage(), pageSize, defaultSortField, false, false);
	return c.RenderJson(notes)
}

// 得到note和内容
func (c Note) GetNoteAndContent(noteId string) revel.Result {
    note_content, _ := noteService.GetNoteAndContent(noteId, c.GetUserId())
	return c.RenderJson(note_content)
}

// 透過 noteId 取得筆記內文
func (c Note) GetNoteContent(noteId string) revel.Result {
	noteContent, _ := noteService.GetNoteContent(noteId, c.GetUserId())
...
func (c Note) UpdateNoteOrContent(noteOrContent NoteOrContent) revel.Result {
	...
	if noteOrContent.IsNew {
		...
		note, _ = noteService.AddNoteAndContent(note, noteContent, myUserId)
...
// clear all trash note
func (c Note) ClearTrash() revel.Result {
    total, notes, _ := noteService.ListNotes(c.GetUserId(), "", true, c.GetPage(), pageSize, defaultSortField, false, false);
    if total > 0 {
        for _, note := range notes {
            //b, _ := json.MarshalIndent(note, "", "	") 
            //revel.INFO.Println(string(b))            
            
            trashService.DeleteTrash(note.NoteId.Hex(), c.GetUserId())
        }
    }
...
func (c Note) SearchNote(key string) revel.Result {
	_, blogs, _ := noteService.SearchNote(key, c.GetUserId(), c.GetPage(), pageSize, "UpdatedTime", false, false)
	return c.RenderJson(blogs)
}

// 通过tags搜索
func (c Note) SearchNoteByTags(tags []string) revel.Result {
	_, blogs, _ := noteService.SearchNoteByTags(tags, c.GetUserId(), c.GetPage(), pageSize, "UpdatedTime", false)
	return c.RenderJson(blogs)
}

// html2image
func (c Note) Html2Image(noteId string) revel.Result {
    revel.INFO.Println("Html2Image")
    
	re := info.NewRe()
	userId := c.GetUserId()
	note, _ := noteService.GetNote(noteId, userId)
	if note.NoteId == "" {
		return c.RenderJson(re)
	}
	content, _ := noteService.GetNoteContent(noteId, userId)
