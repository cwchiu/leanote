...
func (this *ShareService) ListShareNotesByNotebookId(notebookId, myUserId, sharedUserId string, 
	...
	_, notes, _ := noteService.ListNotes(sharedUserId, notebookId, false, page, pageSize, sortField, isAsc, false);
...
//		Sort(sortFieldR).
	...
	_, notes, _ := noteService.ListNotesByNoteIds(noteIds)
...
func (this *ShareService) HasUpdatePerm(userId, updatedUserId, noteId string) bool {
	...
	*/
	if !db.Has(db.ShareNotes, 
		...
		notebookId, _ := noteService.GetNotebookId(noteId)
		if notebookId.Hex() == "" {
			return false
		}
...
func (this *ShareService) hasSharedNotebook(noteId, myUserId, sharedUserId string) bool {
	note, _ := noteService.GetNote(noteId, sharedUserId)
	if note.NoteId != "" {
