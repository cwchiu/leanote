...
//-------------
Notebook.clearTrashNote = function(){
    ajaxGet("/note/ClearTrash", {}, function(ret) {
        if(ret.Ok) {        
            $("#notebookList li a[notebookid=0]").click();
            $("#notebookList li a[notebookid=-1]").click();
...
$(function() {
    ...
    var notebookListMenu = {
        width: 150, 
        items: [
            { text: MSG['shareToFriend'], alias: 'shareToFriends', icon: "", faIcon: "fa-share-square-o", action: Notebook.listNotebookShareUserInfo},
            { type: "splitLine" },
            { text: MSG['publishToBlog'], alias: 'set2Blog', icon: "", action: Notebook.setNotebook2Blog },
            { text: MSG['rollbackFromBlog'], alias: 'unset2Blog', icon: "", action: Notebook.setNotebook2Blog }, // Unset
            { type: "splitLine" },
            { text: MSG['rename'], icon: "", action: Notebook.updateNotebookTitle },
            { text: MSG['delete'], icon: "", alias: 'delete', faIcon: "fa-trash-o", action: Notebook.deleteNotebook }
        ],
    ...
    $(":not(#notebookList li a:last-child)").contextmenu(notebookListMenu);
    
    $("#notebookList li a:last-child").contextmenu({
        width: 150, 
        items: [
            { text: MSG['clearTrashNote'], icon: "", alias: 'delete', faIcon: "fa-trash-o", action: Notebook.clearTrashNote }
        ],
        onContextMenu: function () {
            var notebookId = $(this).attr("notebookId");
            return Notebook.isTrashNotebookId(notebookId);
        },        
        parent: "#notebookList ",
        children: "li a"
    });
    ...
    var addNotebookContextmenu = {
        width: 150, 
        items: [
            { text: MSG['addNotebook'], icon: "", action: Notebook.addNotebook },
        ],
        parent: "#myNotebooks",
        children: ""
    }
